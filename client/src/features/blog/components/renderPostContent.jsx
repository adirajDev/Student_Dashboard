    ...
    return marks.reduce((node, mark) => {
        switch (mark.type) {
            ...
            case 'strike':
                return <s>{node}</s>;
            case 'code':
                return <code className="px-1 py-0.5 bg-[var(--color-ink-100)] rounded text-sm">{node}</code>;
            case 'link':
                return (
                    <a href={mark.attrs?.href} target="_blank" rel="noreferrer" className="text-[var(--color-amber-700)] underline">
                        {node}
                    </a>
                );
            default:
...
const renderNode = (node, key) => {
    ...
    switch (node.type) {
        ...
        case 'blockquote':
            return (
                <blockquote key={key} className="border-l-4 border-[var(--color-amber-300)] pl-4 italic text-[var(--muted)] mb-3">
                    {(node.content || []).map((child, i) => renderNode(child, i))}
                </blockquote>
            );
        case 'horizontalRule':
        ...
        case 'youtube':
            return (
                <div key={key} className="aspect-video rounded-[var(--radius-md)] overflow-hidden my-4">
                    <iframe
