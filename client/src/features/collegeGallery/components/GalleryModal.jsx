...
const GalleryModal = ({ isOpen, onClose, collegeId, images, videos }) => {
    ...
    const allMedia = [
        ...
        ...(videos || []).map((vid, idx) => ({
            ...vid,
            type: 'video',
            index: (images?.length || 0) + idx,
        })),
    ];

    const modalContent = (
        <div className="fixed inset-0 z-[100] flex items-center justify-center surface-paper animate-in fade-in duration-300">
            {/* Close Button */}
            <button
                onClick={onClose}
                className="absolute top-6 right-6 p-2 text-[var(--muted)] hover:text-[var(--foreground)] hover:bg-[var(--color-ink-50)] rounded-full transition-colors z-50"
            >
                <X className="w-8 h-8" />
            </button>

            {allMedia.length === 0 ? (
                <div className="text-[var(--muted)] text-xl font-display">
                ...
                <div className="w-full h-full flex flex-col md:flex-row p-4 md:p-8 gap-6 max-w-[1600px] mx-auto">
                    ...
                    <div
                        className="w-full md:w-48 lg:w-64 flex-shrink-0 flex md:flex-col gap-4 overflow-x-auto md:overflow-y-auto pb-4 md:pb-0 hide-scrollbar"
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative flex-shrink-0 w-24 md:w-full aspect-video md:aspect-square rounded-xl overflow-hidden border-2 transition-all duration-200 ${
                                        isSelected
                                            ? 'border-[var(--color-ink-600)] scale-95'
                                ...
                                >
                                    {media.type === 'image' ? (
                                        ...
                                        <div className="w-full h-full bg-[var(--color-ink-50)] flex items-center justify-center">
                    ...
                    <div className="flex-1 flex items-center justify-center relative rounded-3xl overflow-hidden bg-[var(--color-ink-50)] border border-[var(--border)]">
