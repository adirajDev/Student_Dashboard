...
import { X, User, Mail, Phone, GraduationCap, Building2, Calendar, FileText, Trash2, Plus, Loader2 } from 'lucide-react';
...
const UserDetailsModal = ({ user, onClose, onUserApplicationsUpdate }) => {
    ...
    const selectedCollegeObj = collegesList.find((c) => (c._id || c.id) === newCollegeId);
    const availableCoursesForSelectedCollege = selectedCollegeObj?.availableCourses || [];
                ...
                <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
                    ...
                    {success && (
                        <div className="p-3 bg-green-50 text-green-700 border border-green-200 rounded-2xl text-sm flex justify-between items-center">
                            <span>{success}</span>
                            <button onClick={() => setSuccess('')} className="p-1 hover:bg-green-100 rounded-full">
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                    )}
                        ...
                        <div className="space-y-4 pt-2 border-t border-[var(--border)]">
                            <div className="flex items-center justify-between">
                                <h3 className="text-md font-bold text-slate-900 flex items-center gap-2">
                                    ...
                                    College Applications ({applications.length}/3)
                                </h3>
                            ...
                            {isAdding && (
                                <form onSubmit={handleAddSubmit} className="p-4 rounded-2xl border border-indigo-200 bg-indigo-50/40 space-y-3 animate-fade-in">
                                    <div className="flex justify-between items-center mb-1">
                                        <h4 className="text-xs font-bold uppercase tracking-wider text-indigo-900">New Application</h4>
                                        <button
                                            type="button"
                                            onClick={() => {
                                                setIsAdding(false);
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        ...
                                        <div>
                                            <label className="block text-xs font-medium text-slate-700 mb-1">Course (Optional)</label>
                                            <select
                                            ...
                                            >
                                                <option value="">No Course (Pending)</option>
                                                {availableCoursesForSelectedCollege.map((ac) => {
                                                    const courseObj = ac.course || ac;
                                                    return (
                                                        <option key={courseObj._id || courseObj} value={courseObj._id || courseObj}>
                                    ...
                                    <div className="flex justify-end gap-2 pt-1">
                                        ...
                                        <button
                                            type="submit"
                                            disabled={loadingMap.add || !newCollegeId}
                                    ...
                                    {applications.map((app) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <span className="text-xs text-slate-500 font-medium">Selected Course:</span>
                                                    <select
