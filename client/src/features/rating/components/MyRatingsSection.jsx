...
const MyRatingsSection = () => {
    const { ratings, isLoading, getMyRatings, updateRating, deleteRating } = useRatings();
        ...
        <div className="animate-fade-in">
            ...
            {!ratings || ratings.length === 0 ? (
                <div className="bg-[var(--card)] p-8 rounded-3xl border border-dashed border-[var(--border)] flex flex-col items-center justify-center text-center">
                    ...
                    <h4 className="text-lg font-medium text-[var(--foreground)] mb-2">No Reviews Yet</h4>
                    <p className="text-[var(--ring)] max-w-sm">
                        You haven't shared your experience yet. Visit your college's page to write a review!
                    </p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {ratings.map(rating => (
                        <div key={rating._id} className="bg-[var(--card)] p-6 rounded-3xl border border-[var(--border)] shadow-sm flex flex-col h-full">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <Link to={`/college/${rating.college?._id}`} className="text-lg font-semibold text-blue-600 dark:text-blue-400 hover:underline mb-1 inline-block">
                                        {rating.college?.name || 'Unknown College'}
                                    </Link>
                            ...
                            {rating.comment ? (
                                ...
                                <p className="text-[var(--ring)] text-sm italic mt-2 flex-grow">No comment provided.</p>
                            )}
