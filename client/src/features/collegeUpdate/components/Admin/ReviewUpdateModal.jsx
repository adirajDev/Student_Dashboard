...
const ReviewUpdateModal = ({ update, onClose, onApprove, onReject }) => {
        ...
        return (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-2">
                <div className="bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                    ...
                    <p className="font-medium">{details.averagePackage ? `₹${details.averagePackage} LPA` : 'N/A'}</p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                    <p className="text-xs text-[var(--ring)] uppercase mb-1">Highest Package</p>
                    <p className="font-medium">{details.highestPackage ? `₹${details.highestPackage} LPA` : 'N/A'}</p>
                </div>
                <div className="bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                    <p className="text-xs text-[var(--ring)] uppercase mb-1">Placement %</p>
                    <p className="font-medium">{details.placementPercentage ? `${details.placementPercentage}%` : 'N/A'}</p>
                </div>
            </div>
        );
    };
    ...
    const renderRecruiters = (recruiters) => {
        if (!recruiters || recruiters.length === 0) return <p className="text-sm italic text-[var(--ring)]">No recruiters listed</p>;
        return (
            <div className="flex flex-wrap gap-2 mt-2">
                {recruiters.map((rec, i) => (
                    <span key={i} className="px-3 py-1.5 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800/50 rounded-lg text-sm font-medium">
                        {rec}
                    </span>
                ))}
            </div>
    ...
    const renderFaculty = (faculty) => {
        if (!faculty || faculty.length === 0) return <p className="text-sm italic text-[var(--ring)]">No faculty listed</p>;
        return (
                ...
                {faculty.map((fac, i) => (
                    <div key={i} className="bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-[var(--ring)]">{fac.role} • {fac.department}</p>
                        </div>
                    </div>
                ))}
            </div>
                ...
                <div className="p-6 overflow-y-auto flex-1 space-y-6">
                    ...
                    <div className="space-y-6">
                        ...
                        {changes.placementDetails && (
                            <div className="border border-[var(--border)] rounded-2xl p-5 bg-white dark:bg-slate-800/50 shadow-sm">
                                <div className="flex items-center gap-2 mb-2 text-[var(--foreground)] font-semibold border-b border-[var(--border)] pb-3">
                                    <TrendingUp className="w-5 h-5 text-blue-500" /> Placements (Proposed)
                                </div>
                                {renderPlacementDetails(changes.placementDetails)}
                            </div>
                        )}
                        ...
                        {changes.recruiters && (
                            <div className="border border-[var(--border)] rounded-2xl p-5 bg-white dark:bg-slate-800/50 shadow-sm">
                                <div className="flex items-center gap-2 mb-2 text-[var(--foreground)] font-semibold border-b border-[var(--border)] pb-3">
                                    <Building2 className="w-5 h-5 text-purple-500" /> Top Recruiters (Proposed)
                                </div>
                        ...
                        {changes.faculty && (
                            <div className="border border-[var(--border)] rounded-2xl p-5 bg-white dark:bg-slate-800/50 shadow-sm">
                                <div className="flex items-center gap-2 mb-2 text-[var(--foreground)] font-semibold border-b border-[var(--border)] pb-3">
                                    <Users className="w-5 h-5 text-orange-500" /> Faculty Roster (Proposed)
                                </div>
