...
const SearchableSelect = ({
    ...
    return (
        <div ref={wrapperRef} className="relative w-full">
            ...
            {isOpen && (
                <div className="absolute z-50 w-full mt-1 bg-[var(--card)] border border-[var(--border)] rounded-[var(--radius-xl)] shadow-xl max-h-60 overflow-y-auto animate-fade-in">
                    {filteredOptions.length > 0 ? (
                        filteredOptions.map(opt => (
                            <div
                                key={opt._id}
                                onClick={() => handleSelect(opt._id)}
                                className={`px-4 py-2.5 cursor-pointer hover:bg-[var(--color-ink-50)] transition-colors ${value === opt._id ? 'bg-[var(--color-ink-50)] text-[var(--color-ink-600)] font-medium' : 'text-[var(--foreground)]'}`}
                            >
                                {opt.name}
                            </div>
                        ))
                    ...
                    {customOption && (
                        <div className="border-t border-[var(--border)]">
                            <div
                                ...
                                className={`px-4 py-2.5 cursor-pointer hover:bg-[var(--color-ink-50)] transition-colors ${value === customOption.value ? 'bg-[var(--color-ink-50)] text-[var(--color-ink-600)] font-medium' : 'text-[var(--foreground)]'}`}
                            >
