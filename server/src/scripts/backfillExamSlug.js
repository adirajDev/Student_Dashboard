/**
 * One-off migration: backfill `slug` on every Exam document.
 *
 * Usage:
 *   node src/scripts/backfillExamSlug.js --dry-run     # print, change nothing
 *   node src/scripts/backfillExamSlug.js               # write
 *   node src/scripts/backfillExamSlug.js --regenerate  # also re-slug docs that already have one
 *
...
async function main() {
    ...
    console.log(`${docs.length} exams scanned, ${ops.length} to update.`);
    ...
    const res = await exams.bulkWrite(ops, { ordered: false });
    ...
    try {
        await exams.createIndex({ slug: 1 }, { unique: true, name: 'slug_1' });
