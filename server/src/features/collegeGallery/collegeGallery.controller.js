...
export const addImages = asyncHandler(async (req, res) => {
    if (!checkOwnership(req.user, req.params.collegeId)) {
        return res.status(403).json({ message: 'Access denied. You can only modify your own college.' });
    }
    if (!req.files || req.files.length === 0) {
        return res.status(400).json({ message: 'No files provided' });
    }
    const college = await collegeGalleryService.addImages(req.params.collegeId, req.files);
...
export const deleteImage = asyncHandler(async (req, res) => {
    ...
    const college = await collegeGalleryService.deleteImage(req.params.collegeId, req.params.imageId);
...
export const addVideo = asyncHandler(async (req, res) => {
    ...
    const college = await collegeGalleryService.addVideo(req.params.collegeId, req.body.url);
...
export const deleteVideo = asyncHandler(async (req, res) => {
    ...
    const college = await collegeGalleryService.deleteVideo(req.params.collegeId, req.params.videoId);
    res.status(200).json({ message: 'Video deleted successfully', college });
});

export const serveImage = asyncHandler(async (req, res) => {
    const image = await collegeGalleryService.getImage(req.params.collegeId, req.params.imageId);
