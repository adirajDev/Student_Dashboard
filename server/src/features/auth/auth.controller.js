...
export const resetOtpPassword = asyncHandler(async (req, res) => {
    const user = await authService.resetOtpPassword(req.body.email, req.body.otp, req.body.newPassword);
    generateTokenAndSetCookie(res, user._id);
    res.json({ message: 'Password updated successfully and logged in', user: { id: user._id, email: user.email, name: user.name, role: user.role, isFirstLogin: user.isFirstLogin } });
});
